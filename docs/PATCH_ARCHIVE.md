...
Current AmberELEC packaging changes the `reicast` prefix to `flycast2022`, so
the first two keys are exposed there as `flycast2022_adjacent_state_elision`
and `flycast2022_translucent_strip_merge`. The archived patch names and
historical candidate binaries retain their Flycast 2021 naming.
...
`reicast` option prefix and `Flycast` library name; AmberELEC packages this
fork separately as `Flycast 2022 Low-End` with `flycast2022_*` keys. The
renderer implementation is unchanged by this naming correction.
...
`Flycast` library name; AmberELEC packages the same source as
`Flycast 2022 Low-End` to keep its settings and saves separate from both
modern Flycast and the historical Flycast 2021 package.
